...
const productIds = [
  ...
  'xteink-x4', 'xteink-x3', 'boox-palma-2-pro', 'supernote-nomad',
];

test('all 16 products have working direct routes, images, and purchase links', async ({ page }) => {
  const errors = [];
...
test('complete PaperS3 and C1 Slim photos and added brands are retained', async ({ page }) => {
  ...
  await expect(page.locator('.product-card h3')).toHaveText(['X4', 'X3']);
