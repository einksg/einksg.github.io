...
test('keeps the added products and original desktop spacing', async ({ page }) => {
  ...
  await expect(page.locator('.year-heading').first()).toHaveCSS('font-size', '12px');
  for (const section of await page.locator('.year-section').all()) {
    const year = await section.locator('.year-heading').textContent();
    for (const label of await section.locator('.product-topline > span').allTextContents()) expect(label).toContain(`(${year})`);
  }
  await expect(page.locator('.product-grid').first()).toHaveCSS('row-gap', '56px');
  await expect(page.locator('.product-grid').first()).toHaveCSS('column-gap', '24px');
...
test('keeps the original mobile spacing without card overlap', async ({ page }) => {
  ...
  await page.goto('/');
  await expect(page.locator('.product-grid').first()).toHaveCSS('row-gap', '38px');
  await expect(page.locator('.product-grid').first()).toHaveCSS('column-gap', '14px');
